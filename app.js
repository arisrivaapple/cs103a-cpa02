...
var mailOptionsAttachment = {
  ...
  attachments: [
        {   // file on disk as an attachment
            filename: 'kit.png',
            path: '**/upload/kit.png' // stream this file
...
app.post('/upload/',
  ...
  async (req,res,next) => {
    try {
      ...
      const nextUrl = "/sendConfirmationEmail1/?" + "userID=" + userID + "&" + "taskID=" + taskID + "&uploadName=" + image.name
...
app.get("/sendConfirmationEmail1/",
  isLoggedIn,
  async (req, res, next) => {
    try {
      var taskID = req.query.taskID
      var userID = req.query.userID
      var uploadName= req.query.uploadName
      ...
      if (teamName == null) {
        ...
        var captain_ID = team.captainID
        if (captain_ID == null) {
          team.captainID = userID;
          captain_ID = userID;
          await team.save()
        }
        const captain = await User.findOne({_id: captain_ID});
        captainName = captain.username
      }
      //const captainEmail = captain.email
      completerTeamMemberName = user.username
      const task = await Task.findOne({ _id: taskID });
      taskName = task.task_name
      res.locals.task = task
      task.task_status_completed = true

      await task.save()

      link = "http://localhost:5000/taskCompletionCaptainConfirmation/?" + "completerTeamMemberID=" + userID + "&taskID=" + taskID 

      var completionConfirmationRequestToCaptain = {
        from: 'tirehamburger@gmail.com',
        to: 'theironstarre@gmail.com',
        subject: 'yo Please review task submission',
        text: "Hello Captain " + captainName + ",\n Your " + teamName + " teammate, " + completerTeamMemberName + ", has submitted task: \n" + taskName + "!\n Please confirm completion by reviewing the attached submission, and clicking the below link to accept. \n\n" + link,
        html: "Hello Captain " + captainName + ",\n Your " + teamName + " teammate, " + completerTeamMemberName + ", has submitted task: \n" + taskName + "!\n Please confirm completion by reviewing the attached submission, and clicking the below link to accept. \n\n" + link,
        attachments: [
          {   // file on disk as an attachment
            filename: uploadName,
            path: __dirname + '/upload/' + uploadName //how to reference current folder
            ///filename: fileName,
            //path: "upload/" + fileName // stream this file
          } //more types of attahments demonstrated on : https://www.tutsmake.com/node-js-send-email-through-gmail-with-attachment-example/
        ]
      }
      mail.sendMail(completionConfirmationRequestToCaptain, function (error, info) {
        if (error) {
          console.log(error);
        } else {
          console.log('Email sent: ' + info.response);
  ...
  async (req,res,next) => {
    try {
      ...
      for (const task of tasks) {
        ...
        task.task_upload_url = "/upload/?" + "userID=" + task['userId'] + "&" + "taskID=" + task._id;
...
app.get('/submitTask/', 
  ...
  async (req,res,next) => {
    try {
      var taskID = req.query.taskID
      console.log("submit task route task ID: ")
      console.log("submit task route task ID: " + taskID)
      console.log("submit task route task ID: " + taskID)
      console.log("submit task route task ID: " + taskID)
      var taskID = req.query.taskID
      let thisTask = await Task.findOne({_id:taskID});
      res.locals.task = thisTask
      res.locals.taskID = taskID
      res.locals.task_name = thisTask.task_name
      res.locals.task_description = thisTask.task_description
      res.locals.submitConfirmationUrl = thisTask.submitConfirmationUrl
...
app.get("/taskCompletionCaptainConfirmation/",
//the "/taskCompletionCaptainConfirmation/:userID:taskID", sort of works but only takes the first letter away
  isLoggedIn,
  async (req,res,next) => {
  //referenced TaskBoard
    try {
      var completerUserID = req.query.userID
      var taskID = req.query.taskID
      let taskUpdate = await Task.findOne({_id:taskID});
...
app.post("/joinTeamConfirmed", 
  ...
  async (req, res, next) => {
    ...
    console.log("in join team confirmed: " + team)
    res.redirect("/team");
  }
);
